...
### Feature checklists (OKF-backed)
Per-capability feature-comparison grids live in the OKF, never hardcoded. Define the rows in the capability file and each competitor's marks in that capability's offering files under a `# Features` heading, one `- key: value (optional note)` per line: in a capability file `value` is the row title, in an offering file `value` is one of `yes | partial | no | unknown` (the only allowed vocabulary; `validate_okf.py` soft-warns on others). Marks must be grounded in that offering's existing `# Assessment` / `# Detail` / `# Citations`, never invented, and keep `needs_verification` for review. They project into `capabilities.features` / `offerings.features` (jsonb) plus `offerings.detail` (migration `0005_features_detail.sql`) and render in the drawer via `FeatureMatrix`; glyphs are shape-first and neutral (status stays the only colored field). A capability with no `# Features` shows no grid, and FieldPulse stays `unknown`.
